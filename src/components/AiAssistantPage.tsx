  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (imagePreview) {
      message.content = [
        ...
        { type: 'image', image_url: imagePreview }
      ] as any;
    }
          ...
          >
            ...
            >
              {Array.isArray(m.content)
                ? m.content.map((contentPart: any, partIndex: any) => {
                    if (contentPart.type === 'text') {
                      return <p key={`${index}-${partIndex}`}>{contentPart.text}</p>;
                    }
                    if (contentPart.type === 'image') {
                      return <img key={`${index}-${partIndex}`} src={contentPart.image_image_url} alt="User upload" className="max-w-xs rounded-lg" />;
                    }
                    return null;
                  })
                : (typeof m.content === 'string' && <p>{m.content}</p>)
              }
            </div>
          </div>
        ))}
