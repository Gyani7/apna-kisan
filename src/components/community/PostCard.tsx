...
import { Post, timeAgo, formatCount } from "@/lib/types";
import { FeedItemType } from "@/lib/types";

export default function PostCard({ post }: { post: FeedItemType }) {
  if (post.type !== 'post') {
    return null;
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <div className="font-semibold">{p.author.full_name}</div>
              <div className="text-xs text-muted-foreground">{timeAgo(p.createdAt)}</div>
            </div>
            <div className="mt-1 text-sm text-muted-foreground">@{p.author.username}</div>
          </div>
        </div>
        <p className="mt-4 text-sm">{p.content}</p>
        <div className="mt-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon">
              <HeartIcon className="w-5 h-5" />
              <span className="ml-2">{formatCount(p.likesCount)}</span>
            </Button>
            <Button variant="ghost" size="icon">
              <MessageCircleIcon className="w-5 h-5" />
              <span className="ml-2">{formatCount(p.commentsCount)}</span>
            </Button>
            <Button variant="ghost" size="icon">
